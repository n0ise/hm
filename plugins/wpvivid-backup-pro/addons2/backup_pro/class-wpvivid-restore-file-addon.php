...
class WPvivid_Restore_File_addon
{
    ...
    public function extract($file_name,$root_path,$option)
    {
        ...
        $zip_ret = $archive->extract(PCLZIP_OPT_PATH, $root_path,PCLZIP_OPT_REPLACE_NEWER,PCLZIP_CB_PRE_EXTRACT,'wpvivid_pro_function_pre_extract_callback',PCLZIP_OPT_TEMP_FILE_THRESHOLD,16);
        if(!$zip_ret)
        {
            $ret['result']='failed';
    ...
    public function extract_ex($file_name,$extract_files,$root_path,$option)
    {
        ...
        $archive = new PclZip($file_name);
        $zip_ret = $archive->extract(PCLZIP_OPT_BY_NAME,$extract_files,PCLZIP_OPT_PATH, $root_path,PCLZIP_OPT_REPLACE_NEWER,PCLZIP_CB_PRE_EXTRACT,'wpvivid_pro_function_pre_extract_callback',PCLZIP_OPT_TEMP_FILE_THRESHOLD,16);
        if(!$zip_ret)
        {
            $ret['result']='failed';
            $ret['error'] = $archive->errorInfo(true);
    ...
    public function extract_by_index($file_name,$root_path,$start,$end,$option)
    {
        ...
        $archive = new PclZip($file_name);
        $zip_ret = $archive->extractByIndex($index,PCLZIP_OPT_PATH, $root_path,PCLZIP_OPT_REPLACE_NEWER,PCLZIP_CB_PRE_EXTRACT,'wpvivid_pro_function_pre_extract_callback',PCLZIP_OPT_TEMP_FILE_THRESHOLD,16);
        if(!$zip_ret)
        {
            $ret['result']='failed';
            $ret['error'] = $archive->errorInfo(true);
